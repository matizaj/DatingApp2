...
namespace DatingApp.API.Services
{
    public class DatingRepository : IDatingRepository
    {
        ...
        public async Task<Photo> MakeMainPhoto(int userId)
        {
        ...
        {
            var messages = _context.Messages.Include(u => u.Sender).ThenInclude(p => p.Photos).Include(u => u.Recepient).ThenInclude(p => p.Photos).AsQueryable();
        ...
        {
            var messages = await _context.Messages.Include(u => u.Sender).ThenInclude(p => p.Photos).Include(u => u.Recepient).ThenInclude(p => p.Photos)
                .Where(m => m.RecepientId == userId && m.SenderId == recipientId || m.RecepientId == recipientId && m.SenderId == userId)
